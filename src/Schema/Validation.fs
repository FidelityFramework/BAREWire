...
module Validation =
    ...
    /// Returns a list instead of seq - NTU treats list and seq as distinct types
    ...
    let validateTypeInvariants (typeName: string) (typ: SchemaType) : ValidationError list =
        let rec validateType (path: ValidationContext list) (t: SchemaType) : ValidationError list =
            match t with
