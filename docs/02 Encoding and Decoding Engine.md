...
1. **The contract first.** BARE is how bytes cross; the design-time contract is what is preserved when they do ([Substrate_Formalism](./Substrate_Formalism.md), "Three layers"). The bytes are untagged: no type identifiers, no field names, no dimensions travel, only a case index for a union and a presence byte for an optional. The engine serves the contract by being exact under the mapping both sides hold: a value's case structure and payload types are reproduced on the far side because the far side applies the same schema, and a malformed input is refused rather than misread.
