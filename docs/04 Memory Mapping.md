...
> **Built shape (2026-09-03, Readiness Audit §4 step 6).** `src/Memory/Region.fs` and `src/Memory/View.fs` implement the mapping half over the portable byte model: `Region = { Data: byte array; Offset: int; Length: int }` is a bounded extent, sliced only inside its bounds; `View = { Region; Layout: PeripheralLayout }` reads and writes fields by name at the descriptor's declared offsets, little-endian, through the Encoding tier, refusing a field that is missing, whose representation disagrees, or that does not fit. The layout a view is typed by is the fixed-width descriptor of [08](./08%20Hardware%20Descriptors.md), validated against an ABI by `Hardware.Validator`, not a run-time walk of a schema with boxed values as the samples below show; the native substrate has no reflection and no `obj`. The JavaScript realization is the same record over an `ArrayBuffer`-backed array, the same bounded-extent discipline with no addresses, which is the test that the abstraction is not hardware-shaped. The native memref realization, and the shared-memory and memory-mapped-file constructors below, bind through Fidelity.Platform's OS primitives ([07](./07%20IPC%20Platform%20Specific%20APIs.md)) and remain design; the segment layout they share is declared as a `BufferSchema` in the platform description ([11](./11%20Platform%20Description.md)).
