...
¹ Aspirational on the Arty side until Readiness Audit §4 step 9 lands: the Arty description declares pins, clock, and reset today, and no BRAM or register memory space. The descriptor *types* exist (`src/Hardware/Descriptors.fs`, `src/Platform/Description.fs`); the Arty *value* does not yet use them.
